...
Также для строгого контроля архитектурных принципов
используется собственный eslint plugin _eslint-plugin-relative-paths-slice-fsd_,
...
- `npm run lint:ts` - Проверка ts файлов
- `npm run lint:ts:fix` - Исправление ts файлов
- `npm run lint:scss` - Проверка scss файлов
- `npm run lint:scss:fix` - Исправление scss файлов
...
Запросы на сервер отправляются с помощью Async Thunks и [RTK query](/src/shared/api/rtkApi.ts)
...
### В приложении реализовано переключение между старым дизайном и новым.
